...
- [ ] `--analyze` で出た差異を1件ずつ台帳 (`sync.md`) に正規化した
- [ ] 各差異の権威 (model / code / 収束) と種別 (gap / mismatch / naming / violation) を判定した
- [ ] コードを読んで前提が崩れる発見は、実装前にユーザーへ提示して合意を取った
- [ ] コード修正分は計画にスコープ外と方針分岐を明記した
- [ ] アーキ分岐・破壊的操作は実装前に承認ゲートを通した
- [ ] テスト / lint / build を実走し、結果 (失敗・スキップ含む) を正直に報告した
- [ ] コード変更で閉じた差異は `docs/domain/*.md` の赤付箋・未解決の問いを更新した (モデル再整合)
- [ ] 残ったスコープ外を新しい赤付箋／差異候補として記録した
- [ ] コミットはユーザー依頼時のみ・デフォルトブランチなら先にブランチを切った
