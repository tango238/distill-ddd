...
- **状態と状態遷移**: 下記 Step 3.5 で対話して確定する

### Step 3.5: 状態遷移をユーザーと確定する (intent エクスポートの源)

intent↔structure の差分で機械的に突き合わせられる軸は **イベントと状態遷移**。
状態遷移はコードからの推測ではなく、**この場でユーザーと決めて明示的に記録する**
(これが downstream の検証基準＝オラクルになる)。

各 Aggregate について確認:
- 「この Aggregate は明確な状態 (ステータス) を持ちますか? 持たないなら『状態なし』」
- 「状態は何種類で、初期状態はどれですか?」
- 「どの操作 (公開操作) が、どの状態からどの状態へ遷移させますか?」
- 「その遷移はどの Domain Event を発行しますか?」

確定したら下記テンプレートの正準記法で書く。**この記法を `emit_intent.py` が
そのまま読む**ので、表記を崩さないこと:
...
`: 操作 → イベント` の後半は任意 (分かる範囲で)。状態を持たない Aggregate は
`**状態遷移**: なし` と明記する (検討済みであることを残す)。
...
**状態**: {state1}, {state2}, ... (初期状態を先頭に / 状態を持たないなら「なし」)

**状態遷移**:
- {from} → {to} : `{commandName}` → `{EventName}`
- ...
